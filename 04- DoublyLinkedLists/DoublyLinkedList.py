...
class DoublyLinkedList:
    ...
    def prepend(self, value):
        """Method to add a new node to beginning (head)"""
        new_node = Node(value)
        if self.length == 0:
            self.head = new_node
            self.tail = new_node
    ...
    def pop_first(self):
        """Remove first node of list (head)"""
        if self.length == 0:
             return None
        temp = self.head
        if self.length == 1:
    ...
    def set_value(self, index, value):
        """Add a a node at a specific index"""
        temp = self.get(index)
        if temp :
            temp.value = value
            return True
    ...
    def insert(self, index, value):
        """Insert a new node in a specific index and value"""
        if index < 0 or index > self.length:
            return False
        if index == 0:
